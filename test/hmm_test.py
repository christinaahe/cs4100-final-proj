...
class HiddenMarkovModel(object):
    ...
    state_nums = lambda self: range(1, len(self.real_states) + 1)

    vocab_num = lambda self, n: self.vocab[n - 1]
    vocab_nums = lambda self: range(1, len(self.vocab) + 1)
    ...
    def _normalize_observations(self, observations):
        temp = copy.deepcopy(observations)
    ...
    def _follow_backpointers(self, trellis, start):
        ...
        for t in reversed(range(1, len(trellis[1]))):
    ...
    def train_on_obs(self, observations, return_probs=False):
        ...
        posat = [None] + [
            [0] + [forwards[state][t] * backwards[state][t] / forward_prob
                for t in range(1, len(list(observations))+1)]
            for state in self.state_nums()]
        # xi values
        pot = [None] + [
            [None] + [
                [0] + [forwards[state1][t]
                        ...
                        * backwards[state2][t+1]
                        / forward_prob
                  for t in range(1, len(list(observations)))]
...
def read_observations_from_file(file):
    with open(file, 'r') as f:
...
class TestHMM(unittest.TestCase):
    def setUp(self):
        ...
        self.hmm = make_hmm_from_file(HMM_FILENAME)
        self.obs = read_observations_from_file(OBS_FILENAME)
